...
from modules.wireshark_module import capture_traffic, analyze_pcap_file, get_network_interfaces
...
# ====== ROUTES WIRESHARK ======
@app.route('/scan/traffic', methods=['POST'])
def traffic_capture():
    """Endpoint pour capture de trafic"""
    interface = request.form.get('interface', 'eth0')
    duration = int(request.form.get('duration', 30))
    
    scan_id = f"traffic_{int(time.time())}"
    scan_status[scan_id] = {'status': 'running', 'tool': 'Wireshark', 'interface': interface}
    ...
    flash(f'Capture de trafic démarrée sur {interface} ({duration}s)', 'success')
    return redirect(url_for('scan_status_page', scan_id=scan_id))

@app.route('/wireshark/interfaces')
def list_interfaces():
...
def scan_status_page(scan_id):
    """Page de statut d'un scan"""
    if scan_id not in scan_status:
        flash('Scan non trouvé', 'error')
